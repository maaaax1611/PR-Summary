...
\textbf{Note: }
\begin{itemize}
    \item If all class class conditional densities are Gaussian, the Bayesian classifier becomes a Gaussian classifier.
    \item If all covariance matrices are diagonal, we get a Naive Bayes classifier.
    \item If all covariance matrices are equal and the Identity matrix, we get a Nearest neighbor classifier.
\end{itemize}
