...
\begin{equation}
    \delta_k^{(L)} = - \frac{\partial \epsilon}{\partial net_k^{(L)}} = \frac{\partial \epsilon_{MSE}}{\partial y_k^{(L)}} \cdot \frac{\partial y_k^{(L)}}{\partial net_k^{(L)}} = (t_k - y_k^{(L)}) \cdot f'(net_k^{(L)})
\end{equation}

\textbf{2. Hidden Layers ($l < L$):}
For hidden layers, we do not have target values. The error must be propagated back from the layer above ($l+1$). The sensitivity $\delta_j^{(l)}$ for a neuron $j$ in layer $l$ is calculated as the weighted sum of the sensitivities of the connected neurons $k$ in the next layer $(l+1)$.
...
The sensitivity for the hidden layer is:
\begin{equation}
    \delta_j^{(l)} = f'(net_j^{(l)}) \sum_{k} w_{jk}^{(l+1)} \delta_k^{(l+1)}
\end{equation}
The weight update rule remains structurally the same:
\begin{equation}
    \Delta w_{ij}^{(l)} = \eta \cdot \delta_j^{(l)} \cdot y_i^{(l-1)}
\end{equation}
